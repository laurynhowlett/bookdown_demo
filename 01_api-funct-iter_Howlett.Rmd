...
```{r}
raw_data_visitation <- GET(url="https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")
glimpse(raw_data_visitation)
```

```{r}
upacked_data_visitation <- content(raw_data_visitation, as = "text", encoding = "UTF-8" )
```

```{r}
final_data_visitation <- fromJSON(upacked_data_visitation)
```
...
```{r}
unit_visitation <- function(unitcode, sMonth, sYear, eMonth, eYear){
...
unit_visitation("MEVE",1,2021,12,2022)
```
...
```{r}
vis_ROMO <- unit_visitation("ROMO", 11, 1992, 12, 2021)
```

```{r}
vis_EVER <- unit_visitation("EVER", 11, 1992, 12, 2021)
```

```{r}
vis_THRO <- unit_visitation("THRO", 11, 1992, 12, 2021)
```
...
```{r}
vis_ROMO <- unit_visitation("ROMO", sYear=1980, eYear=2021)
```

```{r}
vis_EVER <- unit_visitation("EVER", sYear=1980, eYear=2021)
```

```{r}
vis_THRO <- unit_visitation("THRO", sYear=1980, eYear=2021)
```
...
```{r}
floop_vis <- vector("list", length = length(parks))
```

```{r}
for (i in 1:length(parks)) {
  floop_vis[[i]] <-
    unit_visitation(unitcode = parks[i], sYear = 1980, eYear = 2021)
}
```

```{r}
parks_vis_floop <- bind_rows(floop_vis)
```
...
```{r}
identical(map_vis, floop_vis)
```

```{r}
parks_vis_map <- bind_rows(map_vis)
```
